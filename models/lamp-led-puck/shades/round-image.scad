// Lamp shade: round with an SVG image projected onto the inside
//
// Connector H: 11.2mm
...
module lamp_shade() {
    ...
    // Chamfered image projected onto the shade wall
    image_radius = outer_diameter / 2 - 2 * wall_thickness + epsilon;
    slice_depth = ($preview ? 3 : 1) * layer_height;
    slices = ceil(wall_thickness / slice_depth);

    for (img_no = [0: image_repeat]) {
    zrot(img_no * 360 / image_repeat)
    up((total_height + strut_size) / 2)
    union() {
        for (delta = [0: slice_depth : wall_thickness])
            color($preview ? "white" : "black")
            cylindrical_extrude(
                    ir=image_radius - delta - slice_depth,
                    or=image_radius - delta + epsilon,
                    size=2 * PI * image_radius) {
